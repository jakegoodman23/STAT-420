...
The following function was used to quickly evaluate the model by using the above functions to display LOOCV RMSE and $R^2$:
```{r}
# returns LOOCV RMSE and R-Squared values
...
In an attempt to further improve model fit, the full three way interaction model with log transformations was re-fit three times, each time with either high leverage observations, high influence observations, or outliers removed from the training data.
