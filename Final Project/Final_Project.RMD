...
- The manual removal of predictors (each was attempted)

Each adjustment did not improve model performance and resulted in a violation of the constant variance assumption. log and polynomial transformations were attempted on other variables, but did not improve model performance and, typically, resulted in a violation of constant variance.

_Unusual Observations_

In an attempt to further improve model fit, the full three way interaction model with log transformations was re-fit three times, each time with either high leverage observations, high influence observations, or outliers removed.  This did not result an any added benefit.

Functions to identify unsual observations:
```{r}
